    ...
    xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            for (let i = 0; i < jsonObject.results.length; i++) {
                ...
                let contactId = contact.ID; // ✅ Make sure this is the correct field name

                let row = table.insertRow(-1);
                row.id = "row" + i;

                row.insertCell(0).innerHTML = `<span id="first_Name${i}">${contact.FirstName}</span>`;
                row.insertCell(1).innerHTML = `<span id="last_Name${i}">${contact.LastName}</span>`;
                row.insertCell(2).innerHTML = `<span id="email${i}">${contact.Email}</span>`;
                row.insertCell(3).innerHTML = `<span id="phone${i}">${contact.Phone}</span>`;
...
function deleteContact(rowIndex, contactId)
{
    ...
    let FirstName = document.getElementById("first_Name" + rowIndex).value;
    let LastName = document.getElementById("last_Name" + rowIndex).value;
